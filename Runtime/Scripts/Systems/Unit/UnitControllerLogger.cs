...
namespace LordSheo.JJTK
{
	public class UnitControllerLogger : MonoBehaviour
	{
		...
		private void Start()
		{
			...
			statSystem.OnAddedEvent += (type, amount) => Log($"[Unit/Stat] Added, {amount}, {type}");
			statSystem.OnRemovedEvent += (type, amount) => Log($"[Unit/Stat] Removed, {amount}, {type}");
			statSystem.OnChangedEvent += (type, amount) => Log($"[Unit/Stat] Changed, {amount}, {type}");
			...
			healthSystem.OnHealedEvent += (amount) => Log($"[Unit/Health] Healed, {amount}");
			healthSystem.OnDamagedEvent += (amount) => Log($"[Unit/Health] Damaged, {amount}");
